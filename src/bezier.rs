...
fn eval_bezier_patch(points: &[[Point3; 4]; 4], u: f32, v: f32) -> Point3 {
    let mut curve = [Point3::default(); 4];
    for i in 0..4 {
        curve[i] = eval_bezier_curve(&points[i], u);
    }

    eval_bezier_curve(&curve, v)
}
...
pub fn build_bezier_patch<'a>(
    ...
    for u in 0..=DIVISIONS {
        for v in 0..=DIVISIONS {
            vertices[u][v] = eval_bezier_patch(
                points,
                u as f32 / DIVISIONS as f32,
                v as f32 / DIVISIONS as f32,
            )
        }
    }
